...
author: Kingmany Lin and Prof Fisk
---
...
FILENAME<-"/Users/joylin/Desktop/CMB 523/breastFNA.csv" 
...
breast_data<-seq(nrow(all_breast_data))
new_vector<-sample(breast_data,100, replace=FALSE )
...
Factors are used to work with categorical variables. They're variables that have a fixed and known set of possible values. They're also useful when you want to display character vectors in a non-alphabetical order. 
```{r}
...
The 16th mtry value had the best accurary for the model. 
...
mismatch_total <-sum(trainDat$Prediction != trainDat$Diagnosis)
...
In my results, there was a bias towards misclassifying the benign tumors compared the the malignant. There was 4 misclassifications of true benign tumors compared to 0 for malignant. This is very serious since someone that doesn't have cancer could potentially undergo treatment. 
...
I believe that it would be better to classify benign tumors as malignant. If a patient were to believe that a cancerous tumor were to be benign tumor, it would give time for the cancer to progress. Once a late diagnosis is made, it would be too late to treat. In a perfect world, misdiagnosis doesn't happen but humans do make errors. Some other information that would be helpful in deciding is the data prevalence. Depending on the population that you're studying from, if you understand how common a certain cancer is, either due to environmental factors, aerosols, etc. it'll help you understand more the possibilities of false negatives and positives. 
...
I think the accuracy would be around 50% since it's either B or M. Since the guessing is all random, there's an equal chance for either I or the model to guess how many tumors are malignant or benign. 
...
I thought this assignment was really fun. I enjoyed being able to run code that would help us sort through the data frame to see how many benign or malignant tumors there are and the misclassifications. I'm not sure if I would be able to know all of the code of my head to do this but I wouldn't mind working more about cancer data frames. 
