...
```{r}
n= 133434232
  #pick a large number for N, around one-hundred million or so
res = runif(n) + 1
```

QUESTION 1: What does the above code do?

ANSWER 1: The above code saves the value of n and uses the runif function to generate random numbers in between 1-133434232. Then it adds 1  to those values and it's stored in our environment as values. 
...
runs[k] <-end_time - start_time#total time is the end time minus the start time
print(runs[k])
}
...
ANSWER 2:
i is the variable in the loop that represents each element within the vector "res." The value of i in the first iteration is 1, the second is 2, and the thirty-third would be the 33rd element within that vector. The loop will run through the entire length of the vector. 
...
```{r}
#write code that runs this code 3 times using a loop, saves the amount of time it took to run each time, then prints the average
runs<- numeric(3)#numeric (x) is a number vector that can hold x amount of elements 
for (k in 1:3) {
start_time <- Sys.time()
log_sum <- sum(log(res))
end_time <- Sys.time()#assign the time we finished to variable
...
#the code did work, it changed all of the a's to A's. 
```
...
i represents each row within the matrix start from 1(e.g.row 1, 2, 3) and j represents the column numbers starting from 1 as well. 
...
This implies that during our first version of code, every iteration of the nested loop would mean that you would have to call the nrow and ncol function again. Repeatedly calling the function again, especially for a large dataset, can be overwhelming for the computer. This is why it took longer to run than the second attempt. 
...
The loop above was executed 1000 times since it was all based on the amount of rows (nrows).  
...
Method 2 was the fastest at replacing all of the lower-case a's with the upper case A. The fastest method had a time of 0.0001399517 seconds while the slowest was 30.0172 seconds. It's almost 200,000 times faster. 
...
```{r}
#Your code here
sum_of_rows<-sum(sequencing_matrix=="T")
print(sum_of_rows)
